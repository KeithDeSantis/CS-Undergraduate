...
''' More aggressive mutation function, probability of 1 if organism fitness is 0, 5% otherwise'''
def fitness_dependant(organism):
    return organism.fitness == 0 or random.randint(0, 99) < 5
...
class GeneticAlgorithm:
    def __init__(self,
                 pop_size,  # does this need to be passed or just derived from initial pop?
                 initial_population,
                 selection_function=basic_fitness_selection,
                 should_mutate_function=good_chance,
...
if __name__ == "__main__":
    initial = [5, 4, 9, 1, 2, 8, 3, 7, 0, 6]
    ga = GeneticAlgorithm(pop_size=10, initial_population=initial, culling_factor=6)
    for i in range(10):
        print(ga.most_fit(i))
    ...
    print("Culling 1")
    ga1 = GeneticAlgorithm(pop_size=10, initial_population=initial, culling_factor=0.5)
    ga1.culling()
    print(ga1.population)

    ga2 = GeneticAlgorithm(pop_size=10, initial_population=initial, elitism_factor=.3)
    ga2.elitism()
    print("Elitism")
    print(ga2.population)
    print(ga2.next_population)
