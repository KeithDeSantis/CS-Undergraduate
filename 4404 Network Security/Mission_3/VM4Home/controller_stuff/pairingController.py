...
#* Ideas
# Could include a port to request dst port checking (would mean our bot would have to sit on an HTTP port, and possibly redirect traffic that is for a different server)
# Structure to track request that have been seen and only all replies that correlate properly... removing requests as they get their responses
...
class SimpleSwitch12(app_manager.RyuApp):
    ...
    def is_valid_http(self, pkt, ethsrc, ethdst):
        ...
        src_port = pkt.get_protocols(tcp.tcp)[0].src_port
        ...
        if ("GET" in payload) or ("POST" in payload) or ("PUT" in payload):
            ...
            #* If valid request, add request to ongoing_HTTP_requests dictionary and increment the num. waiting requests
            if validIPAndPort and endsWithCarriageReturn:
                # Key : ( dst_host, dst_port_of_request, src_host, src_port_of_request) Value: 0
                self.ongoing_HTTP_requests.setdefault((ethdst, dst_port, ethsrc, src_port), 0)
                self.ongoing_HTTP_requests[(ethdst, dst_port, ethsrc, src_port)] += 1 # Each request will have 1 response code
        ...
        # --------- HTTP Communication is a response --------- #
            ...
            if pkt.data.endswith(b"\x0D\x0A\x0D\x0A"):
                ...
                try:
                    ...
                    #* if valid response code, make sure the matching request exists and is waiting for a response
                    if isValid:
                        try:
                            # The number of requests on this channel should be positive
                            if self.ongoing_HTTP_requests[(ethsrc, src_port, ethdst, dst_port)] <= 0:
                                return False
                            self.ongoing_HTTP_requests[(ethsrc, src_port, ethdst, dst_port)] -= 1
                        # If there isn't an ongoing request, something is suspicious
                        except:
                            return False
                    return isValid
            ...
            # Response is the Served HTML
                ...
                #* if valid HTML, ensure the request exists and has received a response code (non negative number of in the dictonary)
                if isValid:
                    try:
                        # The number of requests on this channel shouldn't be negative
                        if self.ongoing_HTTP_requests[(ethsrc, src_port, ethdst, dst_port)] < 0:
                            return False
                    # If there isn't an ongoing request, something is suspicious
                    except:
