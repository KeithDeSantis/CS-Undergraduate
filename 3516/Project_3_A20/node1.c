...
void rtinit1() {
    ...
    printf("\n");
    updateDT1(NODE_ID, neighbor1->NodeCosts);
    sendPkt1(neighbor1->NodeCosts);
...
void sendPkt1(int cost[MAX_NODES]) {
...
void updateDT1(int node, int row[MAX_NODES]) {
    for (int i = 0; i < MAX_NODES; i++) {
        for (int j = 0; j < MAX_NODES; j++) {
            if (i == node) {
                dt1.costs[i][j] = row[j];
...
void rtupdate1(struct RoutePacket *rcvdpkt) {
    ...
    updateDT1(node, rcvdpkt->mincost);
    for (int i = 0; i < MAX_NODES; i++) {
        for (int j = 0; j < MAX_NODES; j++) {
            if (i != NODE_ID && i == node) {
                int costToPivot = neighbor1->NodeCosts[i];
                int pivotToDst = rcvdpkt->mincost[j];
                int costToDst = neighbor1->NodeCosts[j];
                if (costToPivot + pivotToDst < costToDst) {
                    neighbor1->NodeCosts[j] = costToPivot + pivotToDst;
                    sendPkt1(neighbor1->NodeCosts);
                }
            }
        }
